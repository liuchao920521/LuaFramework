...
function hotfix_helper.init()
    
    package.path = UnityEngine.Application.dataPath .. "/Res/Script/Lua/?.lua;" .. package.path;
    print("dataPath  =  "..tostring(package.path))
    hotfix.log_debug = function(s) print(s) end
    hotfix.add_protect(global_objects)
end

function hotfix_helper.hotfix_file(filename)
    -- first time invoke to init
    if not hotfix_helper.is_init then
        hotfix_helper.init();
        hotfix_helper.is_init = true;
    end

    local module_name = string.gsub(filename, "/", "%.");
    hotfix.hotfix_module(module_name);
end

return hotfix_helper;
